/**
 * combine thread and MPI, O(n) to allocate id of works to each thread**/
#include <mpi.h>
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
char *directory_name = "/project/parallel/tao_tang/all_results/results_v13";
...
void i_j_id(int *i_id, int *j_id, int work_number, int start_id, int len)
{
	int i=0,j,count = 0;
	/** id of the first work **/
	...
	count = 0;
	/** id of the vectors corrspoding to each work**/
	while(count < work_number)
	{
		i_id[count] = i;
		j_id[count] = j;
...
void two_block_i_j_id(int *i_id,int *j_id,int work_number,int start_id,int b2_size)
{
	int i = 0;
	for(i = 0; i < work_number; i++)
	{
		i_id[i] = (start_id+i)/b2_size;
		j_id[i] = (start_id+i)%b2_size;
	}
}
...
void *thread_work_seq(void *thrd_arg)
{
	...
	int *i_id = malloc(sizeof(int) * work_this_thread); 
	int *j_id = malloc(sizeof(int) * work_this_thread);
	i_j_id(i_id,j_id,work_this_thread,start_id, N -1);	
	...
	for(k=0; k< interval_number; k++)
	{
		for( i = 0; i < end_id-start_id; i++)
			result[i+start_id] += update_covariance(matrix+i_id[i]*M+ k*interval, matrix+j_id[i]*M+k*interval, interval,mean[i_id[i]],mean[j_id[i]]);				
	}	
	
	for( i = 0; i < end_id-start_id; i++)
	{
		result[i+start_id] += update_covariance(matrix+i_id[i]*M+ interval_number*interval, matrix+j_id[i]*M+interval_number*interval, 
		M%interval,mean[i_id[i]],mean[j_id[i]]);
		result[i+start_id]= result[i+start_id]/(M-1);
		/** corr(x,y) = cov(s,y)/ sqrt(var(x) * var(y)**/
		result[i+start_id]= result[i+start_id]/(sqrt(var[i_id[i]]) * sqrt(var[j_id[i]]));		
	}		
	free(i_id);
	free(j_id);
}
...
void * thread_work_even(void *thrd_arg)
{
	...
	int result_count_buffer = 0;
	
	int *i_id = malloc(sizeof(int) * (v_b+1) * (v_b+1)); 
	int *j_id = malloc(sizeof(int) * (v_b+1) * (v_b+1)); 
	...
	for( iter = 0; iter < steps-1; iter++)
	{
		...
		two_block_i_j_id(i_id,j_id,end_id-start_id,start_id,blocks_size[b2_id]);
		...
		{
			for( i = 0; i < end_id-start_id; i++)
				result[result_count+i+start_id] += update_covariance(block1+i_id[i]*M+ k*interval, block2+j_id[i]*M+k*interval, interval,block1_mean[i_id[i]],block2_mean[j_id[i]]);				
		}	

		for( i = 0; i < end_id-start_id; i++)
		{
			result[result_count+i+start_id] += update_covariance(block1+i_id[i]*M+ interval_number*interval, block2+j_id[i]*M+interval_number*interval, 
			M%interval,block1_mean[i_id[i]],block2_mean[j_id[i]]);
			result[result_count+i+start_id]= result[result_count+i+start_id]/(M-1);
			/** corr(x,y) = cov(s,y)/ sqrt(var(x) * var(y)**/
			result[result_count+i+start_id]= result[result_count+i+start_id]/(sqrt(block1_var[i_id[i]]) * sqrt(block2_var[j_id[i]]));			
		}			

		result_count += pairs_number;
...
//		wait_my_barrier(&barrier,n_threads);	
	...
	if(myid >= numprocs/2)
	{
		...
		i_j_id(i_id,j_id,end_id-start_id,start_id, blocks_size[myid]-1);	
	
		//correaltion of block1
		for(k=0; k< interval_number; k++)
		{
			for( i = 0; i < end_id-start_id; i++)
				result[result_count+i+start_id] += update_covariance(block1+i_id[i]*M+ k*interval, block1+j_id[i]*M+k*interval, interval,block1_mean[i_id[i]],block1_mean[j_id[i]]);				
		}	

	
		for( i = 0; i < end_id-start_id; i++)
		{
			result[result_count+i+start_id] += update_covariance(block1+i_id[i]*M+ interval_number*interval, block1+j_id[i]*M+interval_number*interval, 
			M%interval,block1_mean[i_id[i]],block1_mean[j_id[i]]);
			result[result_count+i+start_id]= result[result_count+i+start_id]/(M-1);
			/** corr(x,y) = cov(s,y)/ sqrt(var(x) * var(y)**/
			result[result_count+i+start_id]= result[result_count+i+start_id]/(sqrt(block1_var[i_id[i]]) * sqrt(block1_var[j_id[i]]));			
		}			
		...
		i_j_id(i_id,j_id,end_id-start_id,start_id, blocks_size[b2_id]-1);		
		int count = 0;
		//correaltion of block1
		for(k=0; k< interval_number; k++)
		{
			for( i = 0; i < end_id-start_id; i++)
				result[result_count+i+start_id] += update_covariance(block2+i_id[i]*M+ k*interval, block2+j_id[i]*M+k*interval,interval,block2_mean[i_id[i]],block2_mean[j_id[i]]);				
		}	

		for( i = 0; i < end_id-start_id; i++)
		{
			result[result_count+i+start_id] += update_covariance(block2+i_id[i]*M+ interval_number*interval, block2+j_id[i]*M+interval_number*interval, 
			M%interval,block2_mean[i_id[i]],block2_mean[j_id[i]]);
			result[result_count+i+start_id]= result[result_count+i+start_id]/(M-1);
			/** corr(x,y) = cov(s,y)/ sqrt(var(x) * var(y)**/
			result[result_count+i+start_id]= result[result_count+i+start_id]/(sqrt(block2_var[i_id[i]]) * sqrt(block2_var[j_id[i]]));			
		}			
...
void *thread_work_odd(void *thrd_arg)
{
	...
	int *i_id = malloc(sizeof(int) * (v_b+1) * (v_b+1)); 
	int *j_id = malloc(sizeof(int) * (v_b+1) * (v_b+1)); 
	i_j_id(i_id,j_id,work_this_thread,start_id, blocks_size[myid]-1);	
	
	printf("start id: %d  end id: %d. process %d\n",start_id,end_id,myid);
	...
	{
		for( i = 0; i < end_id-start_id; i++)
			result[i+start_id] += update_covariance(block1+i_id[i]*M+ k*interval, block1+j_id[i]*M+k*interval, interval,block1_mean[i_id[i]],block1_mean[j_id[i]]);				
	}	

	
	for( i = 0; i < end_id-start_id; i++)
	{
		result[i+start_id] += update_covariance(block1+i_id[i]*M+ interval_number*interval, block1+j_id[i]*M+interval_number*interval, 
		M%interval,block1_mean[i_id[i]],block1_mean[j_id[i]]);
		result[i+start_id]= result[i+start_id]/(M-1);
		/** corr(x,y) = cov(s,y)/ sqrt(var(x) * var(y)**/
		result[i+start_id]= result[i+start_id]/(sqrt(block1_var[i_id[i]]) * sqrt(block1_var[j_id[i]]));			
	}			
