/**
 *  MPI-OMP version,blocking communication**/
#include <mpi.h>
#include <omp.h>
#include <stdio.h>
#include <stdlib.h>
...
char *directory_name = "/project/parallel/tao_tang/all_results/results_v20_20_Sep";
char *mark="";
		...
		int *blocks_size,int *blocks2_id)
{
	...
	for( iter = 0; iter < steps-1; iter++)
	{
		...
		MPI_Sendrecv(block2, M * blocks_size[blocks2_id[iter]], MPI_DOUBLE, receiver_id,TAG1,
...
// 	read_synthetic_data(block1,blocks_vector_id[myid] + 1,blocks_size[myid], M);
	...
	if(myid == 0)
	{
		...
		for( j = 0; j < max_results_size; j++)
			printf("%lf ",results_from_each_process[1][j]);
		puts("");			
