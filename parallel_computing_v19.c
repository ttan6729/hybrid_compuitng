/**
 * same as v18, except using synthetic dataset**/
#include <mpi.h>
#include <omp.h>
#include <stdio.h>
#include <stdlib.h>
...
char *directory_name = "/project/parallel/tao_tang/all_results/results_v19_18_Sep";
char *mark="";
...
void write_results(double **results, int N,struct timeval dt)
{
	...
	sprintf(fp,"%s/result_%dvectors_%dnodes_%dcores_%s.txt",directory_name,N,n_node,numprocs,mark);
	FILE *f = fopen(fp, "w");
...
//							printf(" correaltion matrix[%d][%d] = %lf\n",x,y,correlation_matrix[x][y]);
	...
	sprintf(fp,"%s/core_record_%dvectors_%dnodes_%dcores_%s.txt",directory_name,N,n_node,numprocs,mark);	
	FILE *f = fopen(fp, "a");
