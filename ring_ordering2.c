/**
 * pure MPI version, blocking communication**/
#include <mpi.h>
#include <omp.h>
#include <stdio.h>
#include <stdlib.h>
...
char *directory_name = "/project/parallel/tao_tang/all_results/ring_ordering2_20_Sep";
char *mark="";
...
**/
		...
		if(iter < steps -1)
		{
			...
			if(myid == swapper_id)
			{
				MPI_Sendrecv(block1, M * blocks_size[block1_id], MPI_DOUBLE, receiver_id,TAG1,
				block_buffer, M * blocks_size[blocks2_id[iter+1]],MPI_DOUBLE, (myid+1)%numprocs,TAG1,MPI_COMM_WORLD,&stat);
		
				MPI_Sendrecv(block1_mv, 2*(v_b+1) , MPI_DOUBLE,receiver_id,TAG3,
				block_mv_buffer, 2*(v_b+1) , MPI_DOUBLE,(myid+1)%numprocs,TAG3,MPI_COMM_WORLD, &stat1); 		
			}		
				
			else
			{
				MPI_Sendrecv(block2, M * blocks_size[block2_id], MPI_DOUBLE, receiver_id,TAG1,
				block_buffer, M * blocks_size[blocks2_id[iter+1]],MPI_DOUBLE, (myid+1)%numprocs,TAG1,MPI_COMM_WORLD,&stat);
		
				MPI_Sendrecv(block2_mv, 2*(v_b+1) , MPI_DOUBLE,receiver_id,TAG3,
				block_mv_buffer, 2*(v_b+1) , MPI_DOUBLE,(myid+1)%numprocs,TAG3,MPI_COMM_WORLD, &stat1); 				
			}
