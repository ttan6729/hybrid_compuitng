/**
 * combine thread and MPI, O(n^2) to allocate id of works to each thread**/
#include <mpi.h>
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
	...
	for( iter = 0; iter < steps-1; iter++)
	{
			...
			for(i =0; i < blocks_size[myid]; i++)
			{
				for(j = 0; j < blocks_size[b2_id]; j++)
				{
					if( work_count >= start_id && work_count < end_id)
						result[result_count] += update_covariance(block1+i*M+ k*interval, block2+j*M+k*interval, interval,block1_mean[i],block2_mean[j]);
		...
		for(i =0; i < blocks_size[myid]; i++)
		{
			for(j = 0; j < blocks_size[b2_id]; j++)
			{
				if( work_count >= start_id && work_count < end_id)
				{
					result[result_count] += update_covariance(block1+i*M+ interval_number*interval, block2+j*M+interval_number*interval, 
					M%interval,block1_mean[i],block2_mean[j]);
	...
	if(myid >= numprocs/2)
	{
		...
		for(k=0; k< interval_number; k++)
		{
			for(i =0; i < blocks_size[myid]-1; i++)
			{
				...
				{
					if( work_count >= start_id && work_count < end_id)
						result[result_count] += update_covariance(block1+i*M+ k*interval, block1+j*M+k*interval, interval,block1_mean[i],block1_mean[j]);
		...
		for(i =0; i < blocks_size[myid]-1; i++)
		{
			for(j = i+1; j < blocks_size[myid]; j++)
			{
				if( work_count >= start_id && work_count < end_id)
				{
					result[result_count] += update_covariance(block1+i*M+ interval_number*interval, block1+j*M+interval_number*interval, 
		...
		for(k=0; k< interval_number; k++)
		{
			for(i =0; i < blocks_size[b2_id]-1; i++)
			{
				...
				{
					if( work_count >= start_id && work_count < end_id)
						result[result_count] += update_covariance(block2+i*M+ k*interval, block2+j*M+k*interval, interval,block2_mean[i],block2_mean[j]);
		...
		for(i =0; i < blocks_size[b2_id]-1; i++)
		{
			for(j = i+1; j < blocks_size[b2_id]; j++)
			{
				if( work_count >= start_id && work_count < end_id)
				{
					result[result_count] += update_covariance(block2+i*M+ interval_number*interval, block2+j*M+interval_number*interval, 
